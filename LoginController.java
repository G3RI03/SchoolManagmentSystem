...
public class LoginController {
    ...
    public void initialize() {
        ApplicationContext applicationContext = new ClassPathXmlApplicationContext(Directories.CONFIG_XML);
    ...
    public void handleLoginBtn(ActionEvent event) {
        ...
        if (usersDao.userExist(uName, pass)) {
            ...
            transition.setOnFinished(event1 -> {
                ...
                Parent pane = null;
                try {
                    pane = FXMLLoader.load(Objects.requireNonNull(getClass().getResource(Directories.MAIN_FXML_DIR)));
                } catch (IOException e) {
                ...
                scene.getStylesheets().add(Objects.requireNonNull(getClass().getResource(Directories.JFOENIX_CSS_DIR)).toExternalForm());
    ...
    public void handleSignUpBtn(ActionEvent event) {
        StackPane pane = null;
        try {
            pane = FXMLLoader.load(Objects.requireNonNull(getClass().getResource(Directories.SIGNUP_FXML_DIR)));
        } catch (IOException e) {
            System.out.println(e.getMessage());
            e.printStackTrace();
        }
